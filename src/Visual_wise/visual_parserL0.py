...
# #   2. Primary measure  — from axis_bindings
...
# # FIXES_PATH             = str(_PROJECT_ROOT / "config" / "fixes.json")
# # MEASURES_RESOLVED_PATH = str(_PROJECT_ROOT / "output" / "dashboards" / "risk-dash" / "stage1" / "schema_sections" / "measures_resolved.json")
# # GLOSSARY_PATH          = str(_PROJECT_ROOT / "config" / "glossary.json")
...
# # def _detect_comparison(paired_dax: list[DaxEntry], page: str) -> str:
# #     """
...
# #     # Prefer page-matched role
# #     if "ly" in page_lower:
# #         preferred_role = "yoy_card"
# #     elif "lm" in page_lower:
# #         preferred_role = "mom_card"
# #     else:
# #         preferred_role = None

# #     roles_found = {d.role for d in paired_dax}

# #     if preferred_role and preferred_role in roles_found:
# #         return "YoY % change" if "yoy" in preferred_role else "MoM % change"

# #     # Fallback — any card found
...
# #     to primary_measure via:
# #       Case 1 — peer depends on primary
# #       Case 2 — primary depends on peer
# #       Case 3 — shared source columns
# #     """
...
# #         v_primary_raw  = v_measures[0]
# #         v_primary_name = v_primary_raw.split(".", 1)[-1].strip()

# #         # Skip if same measure
...
# #     Returns a fully populated L0Packet or a skip packet.
# #     """
# #     warnings   = []
...
# #         if chains:
# #             primary_field = {
# #                 "property":   chains[0]["measure_name"],
# #                 "field_type": "Measure",
# #                 "table":      chains[0]["table"]
# #             }
# #             warnings.append("primary field was Column — switched to first measure_chain")
# #         else:
# #             return L0Packet(
# #                 visual_id=vid, title=title, visual_type=vtype,
# #                 page=page, primary_measure="", primary_dax=None,
...
# #     # ── 3. Resolve all_dax (visual's own measures_used) ─────
# #     all_dax   = []
# #     missing_m = []
...
# #     if missing_m:
# #         warnings.append(f"measures not found in MEASURES_RESOLVED: {missing_m}")
...
# #         warnings.append(f"primary measure '{primary_measure}' not resolved")

# #     # ── 4. Paired visuals (multiRowCard + card) ─────────────
# #     # O(1) — pre-computed in build_page_context()
...
# #             if entry is None:
# #                 warnings.append(
# #                     f"paired measure not found: {raw.split('.',1)[-1].strip()}"
# #                 )
# #                 continue
...
# #     comparison = _detect_comparison(paired_dax, page)
...
# #     # ── 8. Page visuals ──────────────────────────────────────
# #     # O(1) — pre-computed page_map lookup
# #     page_visuals = []
...
# #     # ── 9. Peer cards (cross-read candidates) ────────────────
# #     # O(1) — pre-computed in build_page_context()
...
# #     Path:
# #       output/l0_packets/{visual_id}_{safe_title}.json

# #     Skip packets bhi save hote hain —
# #     taaki debug mein pata chale kya skip hua aur kyun.
# #     """
# #     import os
...
# #         print(f"      DAX  : {d.dax[:80]}{'...' if len(d.dax)>80 else ''}")
...
# #     # ── Test on first 3 cardVisuals ─────────────────────────
# #     card_visuals = [
# #         v for v in all_visuals
# #         if v.get("type") == "cardVisual"
# #     ][:3]

# #     print(f"\nProcessing {len(card_visuals)} cardVisuals...\n")
# #     for visual in card_visuals:
# #         packet = build_l0_packet(visual, ctx)   # O(1) lookups
...
# # ============================================================
# # CONFIG PATHS  (adjust relative to your project root)
# # ============================================================

# FIXES_PATH             = "../../config/fixes.json"
# MEASURES_RESOLVED_PATH = "../../output/schema_sections/measures_resolved.json"
...
#     # paired_dax empty tha — measures_resolved se check karo
...
#         # No page preference — check what exists
#         if has_yoy:
#             return "YoY % change"
#         if has_mom:
#             return "MoM % change"
...
#     # all_dax + paired_dax se columns
#     # + recursively upstream deps ke columns bhi
#     for dax_entry in all_dax + paired_dax:
#         # Direct columns
#         for col in dax_entry.columns:
#             if col.raw not in seen_cols:
#                 seen_cols.add(col.raw)
#                 all_columns.append(col)
#         # Upstream dep columns — for derived measures
...
#     # paired_dax empty tha — measures_resolved se
#     # YoY/MoM Card ke columns bhi add karo
#     # (e.g. date[month_of_date] jo time intelligence ke liye hai)
#     if primary_measure and MEASURES_RESOLVED:
...
def build_l0_packet(
    ...
    if primary_field.get("field_type") == "Column":
        ...
        if chains:
            ...
            warnings.append(
                "primary field was Column — switched to first measure_chain"
            )
        else:
            return L0Packet(
                visual_id=vid, title=title, visual_type=vtype,
